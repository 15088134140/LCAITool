...
- 每个功能模块需覆盖单元测试、接口测试、E2E 测试三层
- 测试数据使用独立的测试数据库，不得影响生产/开发数据
...
| 测试层级 | 覆盖范围 | 关键用例 |
|---------|---------|---------|
| 单元测试 | 签到逻辑、连续天数计算、奖励计算 | 连续签到7天奖励计算、断签重置、跨天边界 |
| 接口测试 | POST checkin、GET status | 重复签到返回400、未登录返回401、奖励积分正确 |
| E2E 测试 | 用户签到流程 | 登录 → 进入个人中心 → 点击签到 → 验证积分变化 |

### 15.3 邀请机制

| 测试层级 | 覆盖范围 | 关键用例 |
|---------|---------|---------|
| 单元测试 | 邀请码生成、奖励发放逻辑、每日上限 | 邀请码唯一性、自邀拒绝、达到上限后不再奖励 |
| 接口测试 | 注册带 invite_code、invite/info、invite/list | 有效/无效邀请码、充值后奖励发放 |
| E2E 测试 | 完整邀请流程 | 用户A复制邀请码 → 用户B注册填写 → 验证双方得积分 |

### 15.4 工具评价

| 测试层级 | 覆盖范围 | 关键用例 |
|---------|---------|---------|
| 单元测试 | 评价奖励计算（文字/带图）、7天时限 | 超时不可评价、同一任务不可重复评价 |
| 接口测试 | POST/GET ratings、GET stats、POST useful | 提交评价正确返回、列表分页正常、统计分布正确 |
| E2E 测试 | 评价展示流程 | 进入工具详情页 → 查看评价区 → 验证评分显示 |

### 15.5 通用反馈

| 测试层级 | 覆盖范围 | 关键用例 |
|---------|---------|---------|
| 单元测试 | 反馈状态流转、采纳奖励逻辑 | pending→processing→resolved/adopted 状态合法转换 |
| 接口测试 | 用户端提交/列表、管理端操作 | 提交反馈、管理端回复、采纳发放积分 |
| E2E 测试 | 用户提交反馈 | 填写反馈表单 → 提交 → 验证我的反馈列表 |

### 15.6 系统设置

| 测试层级 | 覆盖范围 | 关键用例 |
|---------|---------|---------|
| 单元测试 | 配置读写、分组过滤 | key-value 读写、AES-256 加解密 |
| 接口测试 | GET/PUT settings、AI providers CRUD | 批量更新配置、新增/删除 AI 提供商 |
| E2E 测试 | 管理端设置页面 | 进入系统设置 → 修改基础信息 → 保存 → 验证生效 |

### 15.7 管理端页面（构思审核/退款管理/Dashboard）

| 页面 | 测试层级 | 关键用例 |
|------|---------|---------|
| 构思审核 | 接口测试 | approve/reject/implement 操作、状态变更 |
| 退款管理 | 接口测试 | 退款处理、积分退还正确性 |
| Dashboard | 接口测试 | stats 返回数据格式正确、数据一致性 |
