...
父代理每次构造子代理 prompt 时，末尾必须加上这些规则：

```text
⚠️ 重要规则：
1. 不要执行 git commit 命令！完成后只需要列出你修改/创建的所有文件路径，提交操作由父代理统一执行。
2. 探索代码时优先使用 CodeGraph（`codegraph_explore` 工具），不要直接用 grep 或逐个 Read 文件；如工具不可用则 fallback 到常规方式。
```
