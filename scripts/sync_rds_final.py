...
def get_create_table_sql(conn, table_name):
    """通过查询 pg_get_tabledef 获取精确的 CREATE TABLE 语句"""
    with conn.cursor() as cur:
        cur.execute(f"SELECT pg_get_tabledef('public.{table_name}'::regclass)")
        result = cur.fetchone()
            ...
            for table_name in tables:
                try:
                    ...
                    if create_sql:
                        ...
                        print(f"  创建表: {table_name} ✗ - 无法获取 SQL")
